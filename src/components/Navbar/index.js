  ...
  render() {
    let brand = (
      <span>
        ...
        />
        <span
          className="hide-on-med-and-down" dangerouslySetInnerHTML={{
        ...
        <li key={index}>
          <Link
            className={(item.active)
                            ? 'active'
                            : null} to={item.id} spy offset={10} smooth onSetActive={this.handleSetActive}
          >
            {item.title}
          </Link>
        </li>
      ...
      <nav className="uppercase row">
        <div className="nav-wrapper">
          <Col s={10} l={10} className="offset-s1 offset-l1 no-padding">
            ...
            <a
              className="button-collapse" href="#" data-activates="nav-mobile" dangerouslySetInnerHTML={{
                __html: toggleIcon,
              }}
            />
          </Col>
