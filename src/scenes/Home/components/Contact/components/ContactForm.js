      ...
      case 'phoneNumber':
        if (!value) {
          ...
          if (!fieldValidationErrors.phoneNumber) {
            ...
            fieldValidationErrors.phoneNumber = phoneNumberValid ? '' : 'Phone number is too short';
          }
        }
        break;
      case 'email':
        if (!value) {
          ...
          fieldValidationErrors.email = emailValid ? '' : `${value} is an invalid email address`;
        }
        break;
      default:
        break;
    ...
    return (
      <form onSubmit={this.submitForm}>
        <Row className="contactForm">
          ...
          />
          <Col
            className="uppercase title center-align"
            s={12}
            style={{
              marginTop: '1rem',
            }}
          >
            <Button
              disabled={!this.state.formValid || this.state.formSubmitted || this.state.sendingFrom}
            >
              CONTACT ME
            </Button>
          </Col>
